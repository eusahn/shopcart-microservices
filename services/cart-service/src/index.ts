...
import { cartImpl } from "./service.js";
...
async function main() {
  ...
  const router = (r: ConnectRouter) => r.service(CartService, cartImpl);
  const server = http.createServer(connectNodeAdapter({ routes: router }));
  server.listen(config.GRPC_PORT, () => log.info({ port: config.GRPC_PORT }, "cart-service listening"));
